...
"""Run an AI coding agent against a PRP - Version 3.0.

Enhanced PRP runner with sub-agent support and improved workflow management.
...
Typical usage:
    ...
    # With specific agent
    uv run PRPs/scripts/prp_runner.py --prp test --agent code-reviewer
    
    # Validate PRP before execution
    uv run PRPs/scripts/prp_runner.py --prp test --validate-only

Arguments:
    --prp-path       Path to a PRP markdown file (overrides --prp)
    --prp            Feature key; resolves to PRPs/templates/{feature}.md
    --model          CLI executable for the LLM (default: "claude")
    --agent          Specific sub-agent to use (e.g., ui-ux-agent, code-reviewer)
    ...
    --validate-only  Only validate the PRP without execution
    --parallel       Enable parallel execution for supported operations
